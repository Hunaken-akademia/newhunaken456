...
if (!DRY && failedRaces > 0) {
  console.error(`失敗レースが${failedRaces}件あるため失敗終了します。PENDINGは失敗件数に含めません。`);
  process.exit(2);
}

if (!DRY && savedRaces === 0 && pendingRaces > 0) {
  console.log(`全${pendingRaces}件が結果未確定（PENDING）のため正常終了します。結果確定後の再実行で保存されます。`);
}
