...
async function main() {
  ...
  // AI買い目は、利用者のブラウザではなく実行基盤上のヘッドレスChromiumで保存する。
  ...
  if (aiTargets.length && AUTH_SESSION_JSON) {
    ...
    const error = `AUTOMATION_AUTH_SESSION_JSON is not set (${aiTargets.length} targets)`;
    console.warn(`AI capture failed: ${error}`);
    aiResults = aiTargets.map((j) => ({ ok: false, venue: j.venue, race: j.race, error }));
  }
  ...
  const summary={
    ...
    aiSaved:aiResults.filter(x=>x?.ok).length,
    aiNg,
    errors:[...scheduleErrors, ...results.filter(x=>!x?.ok)].slice(0,10),
    aiErrors:aiResults.filter(x=>!x?.ok).slice(0,10),
  };
