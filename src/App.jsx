...
async function createStripeCheckoutSession(email) {
  ...
  if (!res.ok || !data?.url) {
    const detail = data?.error || raw.slice(0, 300) || "応答本文が空でした。";
    throw new Error(`決済API ${res.status}: ${detail}`);
  }
...
async function fetchRacerResistanceByCourse(regno, myCourse, days = 365) {
  if (!CORRECTION_TABLE_ENABLED) throw new Error("Supabase ENVなし");
  const course = Number(myCourse);
  if (!Number.isInteger(course) || course < 1 || course > 5) return [];
  const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/racer_resistance_by_course`, {
    method: "POST",
    headers: publicSupabaseHeaders(true),
    body: JSON.stringify({
      p_regno: Number(regno),
      p_my_course: course,
  ...
  if (!res.ok) {
    const txt = await res.text().catch(() => "");
    throw new Error(`racer_resistance_by_course ${res.status}${txt ? `: ${txt.slice(0, 160)}` : ""}`);
  }
  const rows = await res.json();
  return Array.isArray(rows) ? rows : [];
}

async function fetchResistanceBaselineByCourse(myCourse, days = 365) {
  if (!CORRECTION_TABLE_ENABLED) throw new Error("Supabase ENVなし");
  const course = Number(myCourse);
  if (!Number.isInteger(course) || course < 1 || course > 5) return [];
  const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/resistance_baseline_by_course`, {
    method: "POST",
    headers: publicSupabaseHeaders(true),
    body: JSON.stringify({
      p_my_course: course,
  ...
  if (!res.ok) {
    const txt = await res.text().catch(() => "");
    throw new Error(`resistance_baseline_by_course ${res.status}${txt ? `: ${txt.slice(0, 160)}` : ""}`);
  }
  const rows = await res.json();
  return Array.isArray(rows) ? rows : [];
}
...
// ── 展開リハーサル（スタート隊形のモンテカルロ試走・純関数版） ──
//   各艇の平均ST・F持ち・進入・当日評価スコアから、スタート隊形を ITERS 回試走し、
//   決まり手の出現分布を出す。固定シードなので同じレースでは常に同じ結果になる。
//
//   ※ v139 変更点（視聴者フィードバック反映）
//     (1) 攻め手の選定を「4→3→5→6の先着勝ち」から「最も余裕のある艇が攻める」に変更。
//         旧実装は4コースが常に先に判定されるため、4号艇の攻め成功率が構造的に過大だった。
//     (2) 5・6コースからの「まくり差し」、3コースからの「差し」を出現候補に追加。
//         旧実装ではリハーサル側にこれらの決まり手が存在せず、シナリオ（決まり手率ベース）と
//         同じ土俵で比較できなかった。
//     (3) 艇別の攻め出現率（boatPct）と決まり手キー別の出現率（pctOf）を返し、
...
function runTenkaiRehearsal({ slit, scoreByBoat = {}, seedKey = "", iters = 1000 }) {
  ...
  const vals = slit.map((e) => scoreByBoat[e.boat] ?? 0);
  const mn = Math.min(...vals), mx = Math.max(...vals);
  const pow = {};
  slit.forEach((e) => { pow[e.boat] = mx === mn ? 0.5 : ((scoreByBoat[e.boat] ?? 0) - mn) / (mx - mn); });

  const ITERS = Math.max(100, iters);
  const tally = {};
  const winCount = {};
  ...
  for (let i = 0; i < ITERS; i++) {
    ...
    for (const c of [3, 4, 5, 6]) {
      ...
      const need = th + rnd() * 0.03;
      const margin = adv - need;
    ...
    // 差し: 2コースが本命だが、3コースからの差しも一定確率で発生させる。
    let sashiBoat = null;
    for (const c of [2, 3]) {
      const e = byCourse[c];
      if (!e) continue;
      const gap = s[c] - s[1];
      const base = c === 2 ? 0.30 : 0.12;
      const p = Math.max(0, base - gap * 3) * (0.4 + 0.8 * pow[e.boat]) * (1 - 0.5 * pow[c1.boat]);
      if (rnd() < p) { sashiBoat = e.boat; break; }
    }
    if (sashiBoat) { addResult("差し", sashiBoat); continue; }
  ...
  const winPct = slit
    ...
    .sort((a, b) => b.pct - a.pct);
  // 決まり手キー別の出現率（"まくり(4)" → 27.6）。シナリオ選定から直接引ける形にする。
  const pctOf = Object.fromEntries(patterns.map((p) => [p.k, p.pct]));
  // 艇別の頭出現率（逃げ・まくり・まくり差し・差し・抜きの合計）。頭候補の判定に使う。
  const boatPct = Object.fromEntries(slit.map((e) => [e.boat, (winCount[e.boat] / ITERS) * 100]));
  return { iters: ITERS, patterns, winPct, pctOf, boatPct };
}
...
export default function App() {
  ...
  // 選手カードは初期状態を折りたたみにして、必要な艇だけ展開する。
  const [expandedBoatCards, setExpandedBoatCards] = useState({});
  const toggleBoatCard = (boat) => {
    setExpandedBoatCards((prev) => ({ ...prev, [boat]: !prev[boat] }));
  };
  const openAllBoatCards = () => setExpandedBoatCards({ 1: true, 2: true, 3: true, 4: true, 5: true, 6: true });
  const closeAllBoatCards = () => setExpandedBoatCards({});
  ...
  // 舟券収支記録UIは撤廃。既存データ・保存処理は安全のため残す。
  const SHOW_BET_LEDGER = false;
  ...
  const allClear = () => {
    ...
    setResistanceByBoat({}); setResistanceBaseline({}); setResistanceStatus("攻められ耐性：未取得");
  ...
  useEffect(() => {
    ...
    const pairs = [1, 2, 3, 4, 5, 6]
      .map((boat) => ({
        boat,
        regno: Number(racerProfiles?.[boat]?.regNo || 0),
        course: Number(courses?.[boat] || boat),
      }))
      .filter((x) => x.regno);

    if (!pairs.length) {
      setResistanceByBoat({});
      setResistanceBaseline({});
    ...
    Promise.all(periods.map(async (period) => {
      ...
      for (const result of results) {
        nextBaseline[result.key] = result.baselines || {};
        for (const item of result.items) {
          if (!nextByBoat[item.boat]) nextByBoat[item.boat] = {};
          nextByBoat[item.boat][result.key] = { course: item.course, rows: item.rows || [] };
        }
      }
      setResistanceBaseline(nextBaseline);
      setResistanceByBoat(nextByBoat);
      setResistanceStatus(`攻められ耐性：読込済 ${pairs.length}艇（進入別・攻めコース別）`);
    }).catch((e) => {
  ...
  const evaluateRows = (rows) => {
    ...
    if (km || rehPct) {
      ...
      if (rehPct) {
        const rehTop = weighted.filter((c) => (c.reh ?? 0) > 0).sort((a, b) => b.reh - a.reh)[0];
        pushSc(rehTop);
      }
      pushSc(weighted.filter((c) => c.boat === heads[0]).sort((a, b) => b.score - a.score)[0]);
      // 実体のある展開のみ（決まり手率10%以上 or リハーサル5%以上）
      for (const c of weighted) {
        if (top.length >= SC_MAX) break;
        if (c.rate < 10 && (c.reh ?? 0) < 5) continue;
        pushSc(c);
      }
      // それでも埋まらなければスコア上位で補完
      for (const c of weighted) {
  ...
  useEffect(() => {
    const onMessage = async (event) => {
      ...
      if (type !== "hunaken-ai-batch-ready" && type !== "hunaken-ai-batch-error") return;

      const targetDate = String(event.data?.date || "");
      ...
      if (type === "hunaken-ai-batch-ready") {
        ...
        const recs = ["本線", "対抗", "穴", "展リハ"].flatMap((label, index) => {
          ...
          const tickets = Array.isArray(source?.tickets)
            ? source.tickets.slice(0, limit).filter(Boolean)
            : [];
          if (!limit || !tickets.length) return [];
          return [{
            id: `batch_${targetDate}_${targetVenue}_${targetRace}_${label}_${now}_${index}`,
            date: targetDate,
            venue: targetVenue,
            race: targetRace,
            label,
        ...
        setBatchBetProgress((p) => ({
          ...p,
          done: p.done + 1,
          saved: p.saved + (recs.length ? 1 : 0),
          skipped: p.skipped + (recs.length ? 0 : 1),
        }));
      } else {
        setBatchBetProgress((p) => ({
          ...p,
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setBatchBetProgress((p) => ({ ...p, done: p.done + 1, skipped: p.skipped + 1 }));
      predictionSaveRef.current = "";
  ...
  const startBatchBetRecording = async () => {
    ...
    if (aiCaptureQueue.length) {
      setBatchBetStatus("一括記録を処理中です");
      return;
    }
      ...
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 10, color: raceWindsMsg.includes("失敗") ? "#ff8a80" : "#7da3c8", marginTop: 6, lineHeight: 1.5 }}>
            {raceWindsLoading ? "保存済みの風設定を読込中…" : raceWindsMsg || "各レースの風を設定すると、そのレースのAI予想に反映されます。買い目は必要なレースだけ追加してください。"}
          </div>
        ...
        <div style={{
          ...
          <button
            type="button"
            onClick={openAllBoatCards}
            style={{
              padding: "7px 11px", borderRadius: 7, cursor: "pointer",
              background: "#16273c", color: "#cfe0f0",
              border: "1px solid #2c4762", fontSize: 11, fontWeight: 800,
            }}
          >すべて開く</button>
          <button
            type="button"
            onClick={closeAllBoatCards}
            style={{
              padding: "7px 11px", borderRadius: 7, cursor: "pointer",
        ...
        <div style={{ display: "grid", gap: 8, marginBottom: 24 }}>
          {[1, 2, 3, 4, 5, 6].map((b) => {
            ...
            const isExpanded = Boolean(expandedBoatCards[b]);
            return (
            <div key={b} style={{
              display: "grid", gridTemplateColumns: "44px 72px 1fr",
              gap: 8, alignItems: "center",
              background: "#16273c", borderRadius: 10, padding: "8px 52px 8px 10px",
              position: "relative",
            }}>
              <div style={{
              ...
              <div style={{
                ...
                flexWrap: "wrap", padding: "2px 0 4px",
                borderBottom: isExpanded ? "1px solid #1d3149" : "none",
              }}>
                {racerProfiles[b] ? (
                  <>
                    <span style={{
                    ...
                    <span style={{ fontSize: 11, color: "#9db5cc" }}>
                      {courses[b]}C{motors[b]?.ren3 != null ? ` / モーター3連 ${motors[b].ren3}%` : ""}
                    </span>
                    {racerProfiles[b].fl && (
                  ...
                  <span style={{ fontSize: 12, color: "#9db5cc", fontWeight: 800 }}>
                    {courses[b]}コース / 選手データ未取得
                  </span>
                )}
              </div>
              <button
                type="button"
                onClick={() => toggleBoatCard(b)}
                aria-expanded={isExpanded}
                aria-label={`${b}号艇の詳細を${isExpanded ? "閉じる" : "開く"}`}
                style={{
                  position: "absolute", top: 8, right: 8,
                  width: 36, height: 36, borderRadius: 8, cursor: "pointer",
                  background: "#0e1b2c", color: "#9db5cc",
                  border: "1px solid #2c4762", fontSize: 16, fontWeight: 900,
                }}
              >{isExpanded ? "▲" : "▼"}</button>
              {isExpanded && (
                <>
                      ...
                      {nigeSim.trustScore != null && (
                        <div style={{
                          ...
                          <details style={{
                            ...
                            <summary style={{ cursor: "pointer", color: "#dbe8f5", fontWeight: 900 }}>
                              ？ 指標の見方・計算方法
                            </summary>
        ...
        <div style={{ marginTop: 28, borderTop: "1px solid #1d3149", paddingTop: 18 }}>
          ...
          <div style={{ background: "#16273c", borderRadius: 10, padding: "12px 14px", marginBottom: 12 }}>
            ...
            <button
              ...
              disabled={aiCaptureQueue.length > 0}
              style={{
                width: "100%", padding: "11px 14px", borderRadius: 8,
                cursor: aiCaptureQueue.length ? "not-allowed" : "pointer",
                background: aiCaptureQueue.length ? "#405267" : "#5a9e2e",
                color: "#fff", fontSize: 13, fontWeight: 800, border: "none",
                opacity: aiCaptureQueue.length ? 0.75 : 1,
              }}
            >
              {aiCaptureQueue.length
                ? `処理中 ${batchBetProgress.done}/${batchBetProgress.total}`
                : "設定中の点数で1R〜12Rを一括記録"}
            </button>
            {batchBetStatus && (
              <div style={{ fontSize: 11, color: "#9db5cc", marginTop: 8, lineHeight: 1.6 }}>
                {batchBetStatus}
                {batchBetProgress.total > 0
                  ? `　完了${batchBetProgress.done}/${batchBetProgress.total}・記録${batchBetProgress.saved}R・スキップ${batchBetProgress.skipped}R`
                  : ""}
              </div>
            )}
          </div>
